...
O NGINX é o proxy reverso padrão e recebe um único virtual host exclusivo para o domínio informado. O instalador não conhece, altera, desativa nem migra Caddy, Apache, PHP-FPM ou aplicações externas. Em uma VPS livre, instala NGINX, configura o proxy para `127.0.0.1:3000`, emite o certificado Let's Encrypt e habilita a renovação. Em servidor compartilhado, preserva virtual hosts existentes; se 80/443 pertencerem a outro proxy, atualiza somente a aplicação e informa que o encaminhamento público continua sob gestão desse proxy externo.
