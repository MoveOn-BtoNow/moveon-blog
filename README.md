...
O NGINX é o proxy reverso padrão. A instalação traduz para virtual hosts isolados as rotas compartilhadas anteriormente pelo Caddy: MOVE.ON, painel PHP-FPM, stream SSE, realtime/WebSocket e aplicação protegida pelo gateway Vupi. Cada certificado é emitido separadamente pelo Let's Encrypt. A configuração inteira é testada diretamente no origin antes de o Caddy ser desabilitado; qualquer falha restaura os arquivos e o serviço anterior automaticamente. Virtual hosts NGINX alheios ao projeto são preservados.
